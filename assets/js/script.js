...
(function(){const f=()=>{window.scrollTo(0,0);document.documentElement.scrollTop=0;document.body.scrollTop=0};f();'scrollRestoration' in history&&(history.scrollRestoration='manual');window.addEventListener('beforeunload',f);window.addEventListener('load',f);window.addEventListener('pageshow',e=>{e.persisted&&f()});document.addEventListener('DOMContentLoaded',()=>{f();hidePageLoader();setTimeout(()=>{document.body.classList.add('loaded');document.documentElement.style.scrollBehavior='smooth'},200)});let c=0;const i=setInterval(()=>{window.scrollY>0?(f(),c++,c>10&&clearInterval(i)):clearInterval(i)},50)})();

// Page Loader Functionality
function hidePageLoader(){const p=document.getElementById('page-loader');p&&setTimeout(()=>{p.classList.add('hidden');setTimeout(()=>{p.parentNode&&p.remove()},500)},1500)}

// Mobile Navigation Toggle
const h=document.querySelector('.hamburger'),n=document.querySelector('.nav-menu');h&&h.addEventListener('click',()=>{h.classList.toggle('active');n.classList.toggle('active')});document.querySelectorAll('.nav-link').forEach(l=>l.addEventListener('click',()=>{h.classList.remove('active');n.classList.remove('active')}));
...
function showBookingSuccessPopup() {
    ...
    const existingPopups = document.querySelectorAll('.booking-success-popup, .booking-error-popup');
...
function calculatePrice(roomType, nights) {
    ...
    else if (nights >= 3) discount = 0.10;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    roomCards.forEach(card => {
        ...
        const originalPrice = parseInt(priceElement.textContent.replace('₹', '').replace(',', ''));
        
        // Add a small tooltip showing weekly/monthly rates
        const tooltip = document.createElement('div');
        tooltip.className = 'price-tooltip';
        tooltip.innerHTML = `
            <div>Weekly: ₹${Math.round(originalPrice * 7 * 0.85).toLocaleString()}</div>
            <div>Monthly: ₹${Math.round(originalPrice * 30 * 0.75).toLocaleString()}</div>
        `;
...
// Send booking data to Google Sheets
function sendBookingToGoogleSheets(bookingData, form = null, submitButton = null, originalButtonText = null) {
    // Use the Google Apps Script Web App URL from config
    const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwbA42bQ0UQ_Q_jV51AugzFQusQdjDhTf7r8oI92_2GDmBl3gL_my6HhW_Yw5Ughot0/exec';
    ...
    formData.append('price', calculateBookingPrice(bookingData.roomType, bookingData.checkIn, bookingData.checkOut));
        ...
        if (xhr.status === 200) {
            try {
                ...
                if (result.success) {
                    ...
                    showBookingErrorPopup('Error submitting booking: ' + (result.error || 'Unknown error'));
                }
            } catch (error) {
    ...
    xhr.onerror = function() {
        ...
        showBookingErrorPopup('Network error. Please check your connection and try again.');
    };
